...
# Illustrative prediction for the cross-correlation amplitude
# (pipeline test scale; free BAO MLE prefers much smaller residual variance).
THETA_OU    = 0.765
SIGMA_X     = 0.018
SIGMA_8     = 0.811
G_COUPLING  = 5.04e-4
...
def main():
  ...
  Datos utilizados:
    Galaxias DESI LRG: datos reales (public DR1 LRG+ELG clustering.dat.fits)
    SNe Ia Pantheon+:  datos reales (Pantheon+SH0ES.dat)
  ...
  Conclusión:
    ¡Resultado con datos reales públicos (DR1 + Pantheon+)!
  
  Nota: Usando DR1 public LRG+ELG clustering como alternativa real sin login.
""")
