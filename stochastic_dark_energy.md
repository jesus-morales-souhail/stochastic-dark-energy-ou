...
where \(x = \ln(1/(1+z))\). For the redshift range probed by DESI (\(z \lesssim 2.3\)) and Euclid (\(z \lesssim 2.0\)), \(g(z)\) is essentially unconstrained by data. We set \(z_* = 1.5\) (illustrative), with the understanding that this parameter is degenerate with \(\sigma\) and cannot be independently calibrated. For all numerical results in Sections 4–6, \(g(z) \approx 1\) in the observed range, meaning the effective amplitude \(\sigma_X\) absorbs any early-time suppression.
...
$$\ln\mathcal{L} = -\frac{1}{2}\left[\mathbf{r}^\top C_\text{total}^{-1} \mathbf{r} + \ln|C_\text{total}| + n\ln(2\pi)\right],$$

where \(\mathbf{r} = \boldsymbol{\alpha}_\text{obs} - \mathbf{1}\) are the residuals from the \(\Lambda\)CDM prediction (\(\alpha = 1\)).

### 4.2 Results

Using the public DESI DR2 BAO data (arXiv:2503.14738, Table 1), the Maximum Likelihood Estimation (MLE) yields a definitive null result:
...
We construct a HEALPix map (\(N_\text{side} = 32\)) of galaxy overdensities from DESI DR1 LRG catalogs (NGC: 1,476,135 objects; SGC: 662,000 objects; \(0.4 < z < 0.8\)). As a proxy for \(\delta\Omega_\Lambda\), we use Pantheon+ Hubble diagram residuals \(\Delta\mu = \mu_\text{obs} - \mu_{\Lambda\text{CDM}}\), converting to \(\delta\Omega_\Lambda\) via the numerical derivative \(d\mu/d\Omega_\Lambda\).
...
**Important caveat:** With only 67 overlapping pixels, the statistical power is limited. Systematic effects (Galactic dust, imaging systematics) have not been controlled. This is a preliminary motivation for DESI DR2 analysis with imaging weights (`WEIGHT_SYS`), not a confirmed detection.

*Note: This result is based on DESI DR1 and does not affect the DR2 BAO null result reported in this work. The cross-correlation signal is a separate preliminary test that requires DR2 imaging systematics for confirmation.*
...
The OU process predicted positive lag correlations in whitened BAO residuals. For a DESI-like 9-bin grid with the best-fit parameters (\(\theta = 0.765\), \(\sigma_X = 0.018\) from DR1 calibration), the original prediction was:
...
We computed the whitened BAO residuals \(\mathbf{y} = L^{-1}\mathbf{r}\), where \(C_\text{std} = LL^\top\) (Cholesky), using the publicly released isotropic \(\alpha\) values from DESI DR2 (7 bins):
...
For \(N = 7\) bins, the standard error of a correlation coefficient is approximately \(\sigma_\rho \approx 1/\sqrt{N-3} \approx 0.5\), giving a 95% confidence interval of \(\approx \pm 0.98\). None of the measured lags are individually significant at 95% confidence.

The observed alternating sign pattern (\(-0.9579\), \(+0.9198\), \(-0.9195\)) is a known mathematical artefact of the whitening operator when applied to a small sample with a diagonal covariance matrix: when \(\sigma_X = 0\), the OU and QNM kernels vanish, the total covariance reduces to \(C_\text{std}\), and the whitened residuals carry the same oscillatory structure as the raw BAO measurements. This does not indicate any physical correlation.
...
In both cases, the log-likelihood remains identical to the \(\Lambda\)CDM baseline (\(\ln\mathcal{L} = 26.484\)). The DESI DR2 BAO data show no evidence for a stochastic dark-energy component in the form of an OU or QNM process.
...
The MLE drives the stochastic noise parameter to the numerical floor, yielding a conservative 95% CL upper limit of \(\sigma_X < 1.5 \times 10^{-4}\). This limit applies to the specific OU/QNM kernel tested here, assuming a fixed CPL background.
...
The finite-information stochastic framework, in its current OU/QNM formulation, is not favored by DESI DR2 data. Models that postulate spacetime as emergent from a discrete information network (e.g., causal sets, spin networks, or holographic entanglement) must be compatible with the observed smoothness of the vacuum. Specifically, any such model must ensure that fluctuations in \(\Omega_\Lambda\) are suppressed by a factor of at least \(10^{-4}\) relative to the background.

Under standard statistical inference, the current cosmological horizon remains highly smooth and Gaussian, setting an upper limit on the information-theoretic coupling of the quantum vacuum. A definitive detection would require cross-validation by independent probes (Euclid, Rubin, Planck) with orthogonal systematics.
...
**Conclusion:** The stochastic model is not favored by the current DESI DR2 BAO data, and we place a phenomenological upper limit on its amplitude. A definitive falsification would require a simultaneous fit of \(\{w_0, w_a, \sigma_X\}\) and, ideally, the \(>20\) bins of Euclid DR1. This work provides the necessary pipeline and a reference limit for that future analysis.

*Note on systematic false positives:* The interpretation of the null result as a phenomenological upper limit is reinforced by the Bayesian analysis in Appendix G. With only one probe, a detected signal would be intrinsically degenerate with instrumental noise; a definitive detection would require cross-validation by independent probes with orthogonal systematics.
...
Notably, Euclid DR1's narrower redshift baseline (\(z \in [0.9, 1.8]\)) yields a higher Rayleigh frequency limit (\(\omega_{R,\text{min}} \approx 16.2\)) than DESI (\(\omega_{R,\text{min}} \approx 6.66\)), meaning Euclid cannot geometrically resolve intermediate-frequency QNM oscillations that DESI could potentially detect (Appendix D). The decisive contribution of Euclid will be statistical power through \(>20\) bins, not oscillatory frequency resolution.
...
where \(K_1\) is the modified Bessel function of the second kind. Any significant deviation from this form would indicate non-linear self-interactions, time-dependent damping, or a breakdown of spatial isotropy. Our analysis pipeline is ready for this future test.
...
$\delta\ln D_V(z) = -\frac{c}{3H_0 D_{M,0}(z)}\int_0^z \frac{X(z')}{E_0(z')^3}\,dz' - \frac{1}{6E_0(z)^2}\,X(z).$
Defining (\chi(z, z')) via (\delta\ln D_V(z) = \int_0^z \chi(z, z'),X(z'),dz'):
$\chi(z, z') = -\frac{c}{3H_0 D_{M,0}(z)\,E_0(z')^3}\,\Theta(z-z') - \frac{1}{6E_0(z)^2}\,\delta_D(z-z'),$
...
Assuming dark energy is a canonical scalar field (\phi) with (\rho_\Lambda \approx V(\phi)) and (\phi \sim M_\text{Pl}), and using (\sigma_X \sim m_\phi^2 M_\text{Pl} H_0 / (2\pi\rho_\Lambda)), the limit (\sigma_X < 1.5 \times 10^{-4}) implies:
...
The theoretical framework of Axioms A1–A4 (Bekenstein–Hawking entropy, Sorkin mechanism) provides a motivation for the stochastic model. The data do not confirm or refute this framework; they only constrain the amplitude of the stochastic component if it exists.
...
These are formal structures compatible with the data when (\sigma_X \to 0), but they are not derived from the data.
Note: The connection between BMS asymptotic symmetries [8] and the Sorkin fluctuation mechanism is presented as theoretical motivation, not a derived result. A rigorous derivation is deferred to future work.
...
$\Xi(\Delta x, r) \equiv \langle X(x_1, \mathbf{r}_1)\,X(x_2, \mathbf{r}_2)\rangle, \qquad r = |\mathbf{r}_1 - \mathbf{r}_2|,\ \Delta x = x_2 - x_1.$
The field satisfies: (i) spatial isotropy — (\Xi) depends only on (r), not on direction; (ii) microcausality — (\Xi = 0) if (\Delta x^2 - (aH)^2 r^2 > 0); (iii) fluctuation-dissipation balance — linear Langevin equation with damping (\theta) and white noise (\sigma).
...
Consistent correlation structure: The signal must match the theoretical prediction (\Xi(\Delta x, r) \propto K_1(\theta_0\sqrt{\Delta x^2 + (aH)^2 r^2})/\sqrt{\Delta x^2 + (aH)^2 r^2}).
Redshift-dependent amplitude: Signal-to-noise should scale with (S(z)).
...
[
\mathcal{L}{\text{int}} = -\frac{1}{4} B_F(\phi) , F{\mu\nu}F^{\mu\nu},
]
...
The scalar field is trapped in a local minimum or false vacuum ((V' \to 0)), which kills the variation of fundamental constants but reduces the model to a pure cosmological constant, in tension with the dynamical evidence from DESI DR2.
The coupling to the electromagnetic sector is strongly suppressed ((|\zeta| \lesssim 10^{-2})), shifting the problem to the need for a screening mechanism (such as the chameleon or symmetron effect) that decouples the field in high-density environments like the quasar absorption systems probed by ESPRESSO.
...
Si después de subirlo sigues viendo problemas en alguna ecuación específica, dime cuál es y la ajusto.Agregar al chatGrok BuildBetaAcceso anticipado para suscriptores de SuperGrok y X Premium+$ curl -fsSL https://x.ai/cli/install.sh | bashExplorar el límite de RayleighAnalizar la simetría BMSAgregar nota sobre el límiteExpert
