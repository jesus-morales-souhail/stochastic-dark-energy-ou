...
I caution that this result is subject to degeneracies between the stochastic component and the CPL parameters, which cannot be fully resolved with only 7 BAO bins. The primary contribution of this work is a validated analysis pipeline and a benchmark for future analyses with the $>20$ bins of Euclid DR1 (expected H2 2026).
...
Rather than attempting a full quantum-gravity derivation, I work at the phenomenological level and model the late-time fluctuations of $\Omega_\Lambda$ as an Ornstein–Uhlenbeck process in $\ln a$. The key observable consequence is an additive covariance term in BAO distance measurements: a redshift-dependent "precision floor" that cannot be beaten by increasing sample size alone. I test this prediction against public DESI DR1/DR2 BAO products and Pantheon+ supernova data, and I outline the analysis protocol for the upcoming Euclid DR1.

---

## 2. Axiomatic Foundation

I clarify that while the standard DESI DR2 BAO analysis constrains the homogeneous isotropic background metric (preserving spatial symmetries), my stochastic framework models the formal breaking of global time-translation invariance inherent to all expanding FLRW spacetimes. Because the expanding background lacks a timelike Killing vector $\mathcal{L}_\xi g_{\mu\nu} \neq 0$ for $\xi^\mu = (1,0,0,0)$ when $\dot{a} \neq 0$, energy conservation via Noether's theorem is globally broken. This non-conservation provides the theoretical opening for late-time vacuum fluctuations. The Ornstein-Uhlenbeck process treats $\delta\Omega_\Lambda(x)$ not as a modification of the background spatial symmetries, but as a stochastic perturbation fueled by this cosmic time-asymmetry, testing whether the vacuum exhibits measurable variance as it is dragged along the cosmic expansion.
...
With $N \sim 10^{122}$, this gives $\delta\Lambda \sim 10^{-61}$ in Planck units. This motivates the existence of a small, non-zero stochastic component, though I do not claim a full derivation of $\rho_\Lambda$ from this argument.

### Axiom A3: Effective Stochastic Dynamics (OU Closure)

Define $X(x) \equiv \delta\Omega_\Lambda(x)$, where $x = \ln a$ is the logarithmic scale factor. I model $X$ as an Ornstein–Uhlenbeck process:
...
The OU process captures finite memory and yields analytic redshift correlations. I adopt it as a minimal phenomenological closure.
...
where $x = \ln(1/(1+z))$. However, for the redshift range probed by DESI ($z \lesssim 2.3$) and Euclid ($z \lesssim 2.0$), $g(z)$ is essentially unconstrained by data. I set $z_* = 1.5$ (illustrative), with the understanding that this parameter is degenerate with $\sigma$ and cannot be independently calibrated. For all numerical results in Sections 4–6, $g(z) \approx 1$ in the observed range, meaning the effective amplitude $\sigma_X$ absorbs any early-time suppression.
...
**Implication.** The cosmological fluid can exchange energy with the expanding spacetime geometry. Photons redshift; the vacuum energy density can vary. This provides the formal opening for stochastic perturbations of the vacuum sector, which I model through the Ornstein-Uhlenbeck process in Axiom A3.
...
I use the standard isotropic BAO distance proxy:
...
$$
\sigma_{\alpha,\rm floor}(z) \approx \lvert S(z) \rvert \sigma_{\Omega_\Lambda}(z).
$$
...
For the numerical results in Sections 4–6, I use the discrete sensitivity kernel $S(z)$, computed numerically for a flat $\Lambda$CDM fiducial ($\Omega_m = 0.315$, $H_0 = 67.4$ km/s/Mpc). This yields:

| $z_{\rm eff}$ | Tracer | $\lvert S(z) \rvert/\lvert S(0.706) \rvert$ | $\sigma_{\alpha,\rm floor}$ |
|:---------------:|:-------------|:---------------------:|:-----------------------------:|
...
The standard BAO analysis assumes Gaussian residuals with diagonal covariance. My model adds an OU-induced component to the total covariance:
...
$$
\mathrm{Cov}[X(x_i), X(x_j)] = \frac{\sigma^2}{2\theta} \exp\left[-\theta \lvert x_i - x_j \rvert\right],
$$


modulated by the activation factor $g(z)$.

I compute the likelihood using the public DESI DR2 BAO measurements (7 bins from arXiv:2503.14738, Table 1) and the standard diagonal covariance from measurement errors. The OU kernel is computed with the sensitivity kernel $S(z)$ from Appendix A.
...
I construct a HEALPix map ($N_{\rm side} = 32$) of galaxy overdensities from DESI DR1 LRG catalogs (NGC: 1.476M objects, SGC: 662k objects) in the redshift range $0.4 < z < 0.8$. I weight each galaxy by the standard DESI completeness weights.

As a proxy for $\delta\Omega_\Lambda$, I use the Hubble diagram residuals $\Delta\mu(z) = \mu_{\rm obs}(z) - \mu_{\Lambda\rm CDM}(z)$ from the Pantheon+ compilation (1701 SNe Ia). For each SN, I estimate
...
where the denominator is computed numerically using the same fiducial $\Lambda$CDM cosmology. I then map these estimates onto HEALPix pixels.
...
Combining NGC and SGC, I find **67 overlapping pixels** with both LRG galaxies and SNe. The cross-correlation coefficient is
...
I computed the whitened BAO residuals using the publicly released isotropic $\alpha$ values from DESI DR2 (7 bins). The results are:
...
I summarize the status of the model based on the current analysis:
...
Upon release of Euclid DR1, I will:
...
My analysis pipeline is ready for this future test.
...
$$
\Xi(\Delta x, r) \equiv \langle X(x_1, \mathbf{r}_1) X(x_2, \mathbf{r}_2) \rangle, \qquad r = \lvert \mathbf{r}_1 - \mathbf{r}_2 \rvert,\quad \Delta x = x_2 - x_1.
$$
...
By spatial homogeneity, I Fourier transform in $\mathbf{r}$:
...
$$
P_X(\Delta x, k) = \frac{\sigma^2}{2\theta(k)} \exp[-\theta(k) \lvert \Delta x \rvert].
$$
...
$$
\Xi(\Delta x, 0) \propto \frac{e^{-\theta_0 \lvert \Delta x \rvert}}{\lvert \Delta x \rvert^{3/2}},
$$
...
Equation (H.5) imposes a severe fine-tuning constraint on any quintessence model that attempts to explain both cosmic acceleration and the proposed stochastic fluctuations. I consider two limiting cases:
