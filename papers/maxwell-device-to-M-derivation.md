...
propagation ($\lvert \partial_z^2 u \rvert\ll k\lvert \partial_z u \rvert$). The slow envelope obeys the
...
$$
(P_d u)(\mathbf r'_\perp)=\frac{k}{2\pi i d}\int d^2r_\perp e^{i k\lvert \mathbf r'_\perp-\mathbf r_\perp \rvert^2/(2d)} u(\mathbf r_\perp).
$$
...
$\Phi_f=-k\lvert \mathbf r_\perp \rvert^2/(2f)$. A general phase-only SLM is an *arbitrary*
...
$$
u_{\rm out}(\mathbf r'_\perp)=\frac{1}{i\lambda B}\int d^2r_\perp \exp\!\Big[\frac{i}{2B}\big(A\lvert \mathbf r_\perp \rvert^2-2\mathbf r_\perp\!\cdot\!\mathbf r'_\perp+D\lvert \mathbf r'_\perp \rvert^2\big)\Big]u_{\rm in}.
$$
...
The tesseract is the 4-cube $\{\lvert x_i \rvert\le1\}\subset\mathbb R^4$; its boundary has 8
cubic cells. Its Coxeter group $B_4$ (hyperoctahedral, signed permutations of
four coordinates, $\lvert B_4 \rvert=2^4\cdot4!=384$) is real. The question I ask is whether it
