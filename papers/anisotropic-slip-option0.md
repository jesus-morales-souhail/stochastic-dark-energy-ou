...
One-page research note (literature + order-of-magnitude) — not a Boltzmann implementation. Related notes: `sdiff-fundamental-vs-emergent.md`, `notes/desqueezing-relaxation-vacuum-fluctuations-note.md`, `papers/resume.txt`.
...
**Conclusion (preview).** The anisotropic “gap” is a real structural crack in SDiff protection, and $\eta$ is the right kind of observable — but with $\sigma_X\sim 10^{-4}$ it inherits the same amplitude problem as the isotropic residual. It is an additional channel that turns on with amplification, not free amplification.
...
Scripts such as `ou_bao_likelihood.py`, `joint_w0wa_sigma_desi.py`, `profile_sigma_x_desi.py`, `eos_efectiva.py`, and `euclid_*` operate on distances ($D_V$, $D_M$, $D_H$) obtained from $\int dz/H(z)$. That is by design: the primary DESI DR2 product I use here is BAO-only residual smoothness.
...
Numbers below are indicative from public DESI-era MG / growth literature. Before any citation in a paper, I open the original work and copy definition + dataset + fiducial.
...
I do not lump these into a single slogan “GR preferred, no story.” I state each with its own significance.
...
**For this Option 0 note:** this is better than a random growth-index: it is a published DESI-era slip constraint of the type I care about (RSD + lensing).
With $\sigma(\gamma)\sim 0.11$, current sensitivity to $\lvert \eta-1 \rvert$ (or $\lvert \gamma-1 \rvert$) is still $\mathcal{O}(0.1)$, far above the $\sim 10^{-4}$ scale of the $f=1$, $\sigma_X\sim 10^{-4}$ estimate below.

### 3.3 Forecasts on slip $\eta$ (or equivalent $\gamma=\Phi/\Psi$)

Model-independent forecasts combining Euclid-like and DESI-like surveys for anisotropic stress / slip typically quote percent-level reach on $\eta$ (few percent, not $10^{-4}$). Exact $\sigma(\eta)$ depends on survey assumptions; I use $\mathcal{O}(0.03\text{–}0.05)$ as order-of-magnitude, not a hard law.
...
5. Plaza, León & Kraiselburd (2025) — unimodular + DESI DR2 (different science goal: holographic DE /$H_0$); useful for pipeline craft, not for my $\sigma_X$ map.

---

## 4. Order-of-magnitude map $\sigma_X \rightarrow \lvert \eta-1 \rvert$**Runnable bridge:** `python scripts/slip_bridge.py`
...
2. Published growth/MG analyses with DESI-like data are GR-compatible at the $\mathcal{O}(0.1)$ level, which is still too coarse to see $\lvert \eta-1 \rvert\sim 10^{-4}$.
3. $\eta$ remains the conceptually correct crack to watch if a large amplification of vacuum residual ever becomes physical.
4. I do not build homemade Boltzmann codes (Option 1) as the next step. If code is needed later: hi_class / MGCAMB / EFTCAMB (Option 2), with a pre-specified target amplitude from the table above.
...
That statement links this Option 0 note to the desqueezing / Sorkin-seed discussion and keeps “easy slip with Euclid” from becoming a new form of hype.
