...
**Code:** https://github.com/jesus-morales-souhail/stochastic-dark-energy-ou
...
| # | Claim | Evidence |
|---|--------|----------|
| C1 | No preference for stationary OU/QNM residual over smooth BAO residuals | MLE floor; \(\Delta\ln\mathcal{L}=0\) vs \(\Lambda\)CDM baseline; AIC not preferred |
...
| C5 | Gap \(\sigma_0\sim 10^{-61}\to\sim 10^{-5}\) is \(\sim 10^{56}\); linear amplifiers fail audit | `amplifier_audit.py` + §5 |
| C6 | Late freeze-out scan: gain \(=1\); soft avalanche gain \(\sim 2\); only \(N_{\mathrm{eff}}\) redefinition hits amplitude | `results/amplification_routes/VERDICT.md` |
...
| N6 | Optical tesseract / pupil / wavefront lab metaphors are **out of claim set** (exploratory repo) |
